...
The hosted control plane stores a tenant-wide emergency stop as independent
server-owned control state. Agent effective-policy retrieval checks that state
before returning authority, so the stop also covers agents enrolled after an
incident begins. Fleet, deployment, group, and agent stop scopes remain
