...
execution data from error logs. Its first task is a probe Lambda with no AWS
authority or configuration bypass; that task always fails after validating the
complete authority until real target-only provider probes are implemented, so
the synthesized stack cannot currently acquire the lock or mutate IAM. A
