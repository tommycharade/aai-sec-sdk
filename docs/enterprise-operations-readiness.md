...
| Policy lifecycle ambiguity | Active authority is separate from immutable drafts; submission, independent review, rationale, staging and atomic activation are enforced, and group reassignment remains explicit and audited. | Provider-neutral and AWS adversarial contracts; hosted UI lifecycle tests; deployed version-ledger read. | Add customer change-management integration, scheduling and measured endpoint-convergence acceptance. |
...
| Host integration | Claude Code, Codex CLI, and GitHub Copilot use the provider-neutral MCP gateway; all supported profiles share contract tests. | Real Codex CLI acceptance on 2026-08-05 passed 10 project/process observations with zero failures and exposed the missing administrator requirements blocker; the real Claude harness and deterministic integration tests are retained. | Install Codex requirements through MDM and obtain an exit-0 rerun; restore Claude authentication and obtain its exit-0 rerun; run organization-policy and upgrade acceptance. |
