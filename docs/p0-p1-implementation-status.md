...
| P0-04 | Complete agent discovery | Partial | Revision-bound, expiring identity/endpoint/source-control snapshots; fail-closed complete-source denominator; unmanaged, duplicate, leaver and orphan reconciliation; redacted hash-bound export; dedicated Coverage UI; adversarial contracts for authority, replay, malformed input, incompleteness and expiry | Build authenticated IdP/MDM/source-control connector services, replace the 100-record pilot snapshot bound with paginated atomic generations, and prove at least 95% real pilot discovery coverage |
