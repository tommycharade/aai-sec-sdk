...
| P0-04 | Complete agent discovery | Partial | Source-scoped revocable connector credentials; redacted operator source directory; UI-first registration, rotation/revocation and separate credential/evidence posture; private exact-version S3 pages with digest-bound atomic commit and a 20,000-record bound; fail-closed complete-source denominator; unmanaged, duplicate, leaver and orphan reconciliation; Entra/endpoint/GitHub reference collectors; AWS-managed Entra, Intune and GitHub scheduling; canonical Intune-to-Entra device-registration identity retained for future online targeting; administrator-run exact binary/process sensor; digest-bound macOS MDM package builder with a fixed root launch daemon, protected per-device credential files and atomic report output; hosted per-device credential lifecycle, HMAC ingestion, replay/tamper denial and server-derived freshness UI; Intune device evidence alone cannot satisfy installation evidence; adversarial contracts for forged credentials, replay, altered hashes, cross-tenant object pointers, unsafe files, stale/revoked/cross-device reports, incompleteness and expiry | Independently build and sign the production sensor executable, package and deploy it through a real customer MDM, complete successful real Entra, Intune and GitHub collections, independently prove provider/device credential scope, prove at least 95% real pilot discovery coverage, and load-test the customer's target above the 20,000-record synchronous envelope if required |
...
| P0-06 | Entra SSO, SCIM and granular RBAC | Partial | Tenant-specific Entra OIDC; tenant-bound SCIM lifecycle and canonical-role mapping; five-minute token reconciliation; expiring canonical/custom-role delegation at tenant, business-unit, project, environment and deployment scope with live revocation, retirement invalidation and scoped reads; immutable safe-capability custom roles with independent review and digest/revision binding; cross-organization group authority-edge denial; recent-MFA, four-eyes, maximum-60-minute break glass; schema-v3 digest-bound access export; persistent secret-free deployment manifest with tenant-specific OIDC/secret/AAI-tenant preflight and missing-manifest removal guard; adversarial contracts, deployed synthetic scope acceptance and focused Identity & Access workspaces for setup, directory roles, custom roles, delegation, emergency access and reviews | Supply and configure the pilot tenant; prove real OIDC and SCIM joiner/mover/leaver; run deployed two-person MFA, delegated multi-business-unit/environment, custom-role retirement and certification exercises |
...
| P0-12 | Assurance package | Partial | Apache-2.0 project; SBOM/release provenance; release-bound machine-validated customer assurance manifest with technical owner and maximum-120-day review; stable-release support policy with a normal 30-day patch overlap; machine-readable calendar-time vulnerability targets with fail-closed review expiry and structured synthetic intake, CVSS, exception, notification and closure-evidence rehearsal; evidence-linked guarantees/non-guarantees; reconciled data-processing/subprocessor schedule; honest penetration-test/SOC 2/ISO status and dated roadmap | Independent penetration test with critical/high closure; real staffed incident-response exercise and fallback intake; immutable `1.1.0` release evidence; legal/privacy approval and signature of customer-specific DPA/commercial terms; qualified framework mapping and independent readiness/audit evidence |
...
| --- | --- | --- | --- |
| Fleet lifecycle | Partial | Enrollment, groups, revision-bound bulk assignment, trusted dynamic-group preview/apply and monitored five-minute deterministic reevaluation, health, immutable desired/package rollout binding, deterministic canary rings, time-zone maintenance windows, server-derived endpoint convergence, automatic health/deadline pause, exact known-good rollback, drift, emergency stop, irreversible revoke, atomic replacement, evidence-retaining offboarding, accountable ownership, server-clock-expiring exact-agent policy exceptions, source-reconciled orphan/leaver detection, AWS-managed Entra/Intune/GitHub discovery connectors, signed endpoint installation/process/platform collection, per-device credential lifecycle, server-derived evidence health, deployment-owned approved runtime-release and immutable delivery-package catalogs, tenant-wide bijective device/install/agent preflight, tenant-scoped version-compliance reporting, revision-bound dual-version runtime canary selection/admission with measured pause and rollback, and a disabled-by-default isolated Intune worker with transactional FIFO outbox, online reauthorization, exact dedicated-group convergence, revision-bound continuation through 500 targets, reproducible maximum-cohort synthetic evidence and provider evidence that cannot replace attestation | Publish real release manifests and approved platform packages; approve and run the bounded Intune pilot; connect physical MDM distribution; complete real-provider population coverage and response automation |
...
| Reporting and administration | Partial | Seven-workspace operator navigation with contextual destinations and preserved deep links; immutable independently reviewed safe-capability custom roles; live delegated canonical/custom authority at tenant, business-unit, project, environment and deployment scope with revision/digest binding and retirement invalidation; schema-v3 access certification; fleet posture, health, SLO and compliance summaries; fail-closed population coverage; deployment-owned approved SDK/gateway/hook catalog and server-derived per-agent/deployment version compliance; purpose-specific executive and evidence-reader assurance reports; operator and daily/weekly scheduled snapshots with domain-separated KMS signatures, exact replicated Object Lock versions, audit-before-state evidence, revision-bound SQS workers, recovery-cell parity and in-product verification/download; scoped expiring service identities; versioned Terraform provider; tenant-managed signed webhooks with durable delivery and replay-verification helper; persistent customer-managed data-key, approved retained-data Region and exact PrivateLink VPC-endpoint authority with a read-only posture UI | Run live multi-business-unit/environment custom-role approval, denial and retirement acceptance; customer-validated framework mappings, external report delivery, real-workload machine-API and Terraform acceptance, provider Registry release/signing, live customer webhook interruption/replay acceptance, live CMK/deletion/residency acceptance, and customer VPN/Direct Connect, DNS, endpoint-policy and security-group acceptance for private access |

### P1-ADM-06 implementation evidence

The control plane now supports immutable custom roles composed only from eight
code-owned safe capabilities. A role starts as an inactive draft, and its
author cannot approve or reject it. Independent approval binds a stable
capability digest and revision. Every custom grant stores that exact revision
and digest, while every authorization reloads the live role; malformed state,
tampering or retirement therefore removes authority on the next request.
Custom roles never enter Entra/Cognito groups and cannot contain wildcard,
identity, integration, runtime, discovery, provider or machine authority.

Delegated grants now resolve tenant, organization (the business-unit boundary),
project, exact environment and deployment scopes from tenant-owned state.
Every lineage contains the authenticated tenant, exact-environment matching is
server-owned, and batches require every target to be covered. Legacy canonical
grants remain compatible. Schema-v3 access certification includes custom-role
lifecycle and exact grant bindings.

The Identity & Access UI provides a table-first custom-role workspace, typed
capability composition with contextual help, human-readable effective
authority, independent decision controls, retirement impact, and one delegated
assignment flow across all five scope types. Contract tests cover positive,
cross-scope, self-approval, wildcard, forged-claim, tamper and immediate
retirement paths. This implements the software foundation of P1-ADM-06. Live
two-person Entra, multi-business-unit/environment and post-retirement denial
acceptance remains required in the pilot tenant; see
