...
| P0-05 | Runtime attestation | Partial | Typed Claude/Codex measurement, deployment-owned manifest validation, nonce-bound heartbeat, baseline drift detection, quarantine/session revocation and adversarial contracts | Pin independently verified release manifests, surface fleet posture in the UI, and complete live modified-package/hook/config/process plus hardware-backed identity acceptance |
