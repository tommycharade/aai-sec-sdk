...
| P0-03 | Native-control reconciliation | Partial | Deterministic deny-first reconciliation, protected-file measurement, plus a release-pinned bounded Codex app-server probe that reports process-loaded approval/sandbox/requirements/hook evidence through the AWS heartbeat and UI | Make the process evidence an execution-authority gate, close Codex MCP/rules/deny-read observability with authorized live acceptance, and obtain an equivalent supported Claude interface or retain explicit manual evidence |
