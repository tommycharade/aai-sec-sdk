...
| Security operations | Partial | Approvals, audit timeline, independent scoped emergency stops, scheduled server-derived endpoint detections, independently reviewed alert-only tool/MCP/denial/approval/decision-volume/outside-project/configuration-error behavior rules with complete-history gates and explainable thresholds, independently re-hashed repository mapping and managed-configuration/runtime-attestation integrity rules with explicit blind spots and no automatic containment, private exact-version S3 integrity baselines with digest binding and invocation-local verified caching, stable evidence-preserving deduplication groups, exact maximum-seven-day suppressions with audited expiry/revocation, audited acknowledgement, durable SNS/SQS and signed-webhook delivery, revisioned cases, source-specific server-revalidated agent binding, evidence-preserving agent quarantine, independently approved endpoint-response rules with preview, action limits, cooldown, idempotent evidence, disable and rollback, session revocation, recovery-gated release and integrity-verifiable content-minimised case export | Credential-broker response, discovery ingestion beyond the documented 2,000-observation generation bound, MDM/EDR isolation and external workflow integrations |
