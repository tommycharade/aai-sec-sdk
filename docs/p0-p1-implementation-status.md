...
| Policy governance | Partial | Typed editor, immutable version ledger, readable active-versus-pending authority, independent review with rationale, self-approval denial, semantic authority diff, bounded redacted historical simulation with explicit indeterminacy, staging, atomic activation, provider-neutral exact-version restrictive composition with reference and AWS signed-control-plane parity, tenant-bound asymmetric signed bundles with locally pinned verification, independently approved KMS-signed temporary agent exceptions with automatic secure restoration, assignment impact, percentage canary/scheduling, evidence-only convergence and deterministic known-good rollback | Add the reviewed Git provenance adapter/workflow and effective-policy UI, then complete live Git-provider and physical-endpoint rollout-SLO acceptance |
