...
| P0-08 | Durable evidence | Partial | S3 Object Lock and cross-region pilot evidence; increase-only 365–3,650-day tenant retention applied to existing and future versions; exact-version tenant-bound legal hold; live byte-integrity assurance; fail-closed complete bounded export with independent browser digest verification; focused Evidence workspace and adversarial contracts | Replace the 250-version synchronous pilot boundary with asynchronous inventory/export, schedule evidence-gap detection and alerts, then repeat live cross-region count/order/hash/retention recovery against the customer environment |
