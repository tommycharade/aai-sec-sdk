...
| P0-03 | Native-control reconciliation | Partial | Deterministic deny-first reconciliation, protected-file measurement, plus a release-pinned bounded Codex app-server probe whose short-lived evidence is bound server-side to the current managed bundle; missing/stale/conflicting/partial proof blocks governed Codex authority while preserving the attested repair route; fixed blocker and effective-action evidence are visible in the UI | Close Codex MCP/rules/deny-read observability with authorized live acceptance, prove matching live execution, and obtain an equivalent supported Claude interface or retain explicit manual evidence |
...
| P0-09 | Production credential broker | Partial | Provider-neutral Azure/GCP workload brokers; scoped AWS STS; exact principal/audience/tool/resource/scope/TTL binding; live revocation checks; secret-free tenant-isolated hosted registration; machine-only fresh evidence; server-owned revocation epoch; typed operator workspace and adversarial contracts | Supply reviewed real AWS/Azure/GCP roles and accounts; deploy token-exchange/revocation adapters; prove least privilege, expiry, outage and revocation against each provider; independently review role policy and retained evidence |
