...
| P0-12 | Assurance package | Partial | Apache-2.0 project; SBOM/release provenance; current stable-release support policy with a normal 30-day patch overlap; machine-readable calendar-time vulnerability targets with fail-closed review expiry and structured synthetic intake, CVSS, exception, notification and closure-evidence rehearsal; draft buyer-facing guarantees/non-guarantees, reconciled data-processing/subprocessor schedule and dated SOC 2/ISO 27001 roadmap | Independent penetration test with critical/high closure; real staffed incident-response exercise and fallback intake; immutable `1.1.0` release evidence; legal/privacy approval and signature of customer-specific DPA/commercial terms; independent readiness/audit evidence |
...
| Security operations | Partial | Approvals, audit timeline, independent scoped emergency stops, scheduled server-derived endpoint detections, independently reviewed alert-only tool/MCP/denial/approval/decision-volume/outside-project/configuration-error behavior rules with complete-history gates and explainable thresholds, independently re-hashed repository mapping and managed-configuration/runtime-attestation integrity rules with explicit blind spots and no automatic containment, private exact-version S3 integrity baselines and discovery pages with digest binding, stable evidence-preserving deduplication groups, exact maximum-seven-day suppressions with audited expiry/revocation, audited acknowledgement, durable SNS/SQS and signed-webhook delivery, revisioned cases, source-specific server-revalidated agent binding, evidence-preserving agent quarantine, independently approved endpoint-response rules with preview, action limits, cooldown, idempotent evidence, disable and rollback, session revocation, recovery-gated release and integrity-verifiable content-minimised case export | Credential-broker response, customer-specific capacity proof beyond the documented 20,000-observation synchronous bound, MDM/EDR isolation and external workflow integrations |
| Reporting and administration | Partial | Fleet posture, health, SLO and compliance summaries; fail-closed population coverage; purpose-specific executive and evidence-reader assurance reports; operator and daily/weekly scheduled snapshots with domain-separated KMS signatures, exact replicated Object Lock versions, audit-before-state evidence, revision-bound SQS workers, recovery-cell parity and in-product verification/download; scoped expiring service identities; versioned Terraform provider; tenant-managed signed webhooks with durable delivery and replay-verification helper | Customer-validated framework mappings, external report delivery, real-workload machine-API and Terraform acceptance, provider Registry release/signing, live customer webhook interruption/replay acceptance, CMK/residency and private access |
